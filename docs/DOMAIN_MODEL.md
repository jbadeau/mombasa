...
### 9.1 The expansive world (chunked, multi-scale)

To scale beyond a bounded map (`DESIGN.md §7.4`), the geography gains a tier
**above** District and is served in **chunks** at a **level of detail**, not as
one snapshot. The world is generated by a **deterministic, seeded** layout
function so the same event history always yields the same world (preserving
"can't fake the city" + playback).

```kotlin
@JvmInline value class RegionId(val value: UUID)
@JvmInline value class ChunkId(val value: Long)          // packed (cx, cy) grid cell

// Hierarchy: World → Region/City → District → Street → Plot → Building
data class Region(val id: RegionId, val name: String, val bounds: BBox)

// Global, effectively unbounded coordinate space; CHUNK_SIZE plots per side.
data class WorldCoords(val x: Long, val y: Long)
data class BBox(val min: WorldCoords, val max: WorldCoords)
enum class Lod { OVERVIEW, DISTRICT, STREET, BUILDING }   // far → near; drives representation

// What the chunked tile endpoint returns — only the plots in one cell at one LOD.
data class WorldChunk(
    val id: ChunkId, val regionId: RegionId, val bounds: BBox, val lod: Lod,
    val plots: List<Plot>,                 // populated at STREET/BUILDING LOD
    val summary: ChunkSummary?,            // populated at OVERVIEW/DISTRICT LOD (counts, health)
)
data class ChunkSummary(val buildingCount: Int, val health: Double, val dominantType: PlotType)
```

- **Read API:** `GET /city/chunks?bbox=…&lod=…` (replaces the single
  `GET /city` for the expansive view); WebSocket subscribes per chunk
  (`city:chunk:{id}`), not one global `city` channel.
- **Spatial index:** plots carry `WorldCoords`; the backend indexes them
  (PostGIS or a grid index) so bbox/chunk queries are fast.
- **Procedural growth:** a seeded `WorldLayout` allocates regions/districts/
  streets/plots from activity counts; real homes/apps/waves then occupy them.
- The **`City`/`DistrictView` shapes above remain** the small-scale / single-
  district view (Phase 2 §7.3); `WorldChunk` is the expansive view (§7.4).
